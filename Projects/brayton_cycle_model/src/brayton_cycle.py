from fluid_properties import get_cp_cv
from initial_conditions import P1, P2, P3, P4, T1, T3, volumetric_flow_rate
...
def main():
    ...
    # Retrieve Cp an Cv
    properties = get_cp_cv(T1)  # This returns a dictionary
    ...
    T2, Wc = isentropic_compression(T1, cp, gamma, rc)
    print(f"Stage 1 - Compression Stage:\nT1= {T1:.2f} K | T2 = {T2:.2f} K | Work input = {Wc:.2f} kJ/kg\n")

    # ------------------------------------------- Process 2: Isobaric Heat Addition ------------------------------------------------------
    # Retrieve Cp an Cv
    properties = get_cp_cv((T2 + T3)/2)  # Cp and Cv at average temperature across the process
    ...
    if isinstance(properties, dict):
        ...
        print("Temperature not found in database.")
        return
    T4, Wt = isentropic_expansion(P3, P4, T3, cp, gamma)
    print(f"Stage 3 - Expansion Stage:\nT4 = {T4:.2f} K | Work Output = {Wt:.2f} kJ/kg\n")

    # --------------------------------------------- Process 4: Isobaric Heat Rejection --------------------------------------------------
    # Retrieve Cp an Cv
    properties = get_cp_cv((T1 + T4)/2)  # This returns a dictionary
    ...
    efficiency = Wnet_work_based / Qin
