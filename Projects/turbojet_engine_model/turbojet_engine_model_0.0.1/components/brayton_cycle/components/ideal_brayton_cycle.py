from libraries.thermodynamic_processes import isentropic_compression, isobaric_heat_addition, isentropic_expansion, isobaric_heat_rejection

def ideal_brayton_cycle(rc, P3, P4, h_1, pr_1, h_3, pr_3):
    """
    ...
    Args:
        ...
        h_1 (float): Enthalpy at the beginning of compression (isentropic).
        pr_1 (float): Pressure ratio at the beginning of compression (isentropic).
        h_3 (float): Enthalpy at the beginning of expansion (isentropic).
        pr_3 (float): Pressure ratio at the beginning of expansion (isentropic).
    ...
    h_2_s, Wc_s  = isentropic_compression(rc, h_1, pr_1) # xxx_s for ISENTROPIC variables
    print(f"Stage 1 - Compression Stage:\nh_1= {h_1:.2f} K | h_2_s = {h_2_s:.2f} K | Work input (ideal) = {Wc_s:.2f} kJ/kg\n")

    # ------------------------------------------- Process 2: Isobaric Heat Addition ------------------------------------------------------
   
    Qin_s = isobaric_heat_addition(h_2_s, h_3)
    print(f"Stage 2 - Heat Addition Stage:\nh_3 = {h_3:.2f} K | Heat Input (ideal) = {Qin_s:.2f} kJ/kg\n")

    # -------------------------------------------- Process 3: Isentropic Expansion ------------------------------------------------------
    
    h_4_s, Wt_s = isentropic_expansion(P3, P4, h_3, pr_3)
    print(f"Stage 3 - Expansion Stage:\nh_4_s = {h_4_s:.2f} K | Work Output (ideal) = {Wt_s:.2f} kJ/kg\n")

    # --------------------------------------------- Process 4: Isobaric Heat Rejection --------------------------------------------------
    
    Qout_s = isobaric_heat_rejection(h_1, h_4_s)
