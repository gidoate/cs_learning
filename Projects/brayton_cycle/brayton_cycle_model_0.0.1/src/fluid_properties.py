...
def get_cp_cv_h(Tactual):
    # Connect to the database
    conn = sqlite3.connect("db/ideal_gas_properties.db")
    cursor = conn.cursor()

    # Query to get all temperature, Cp, and Cv values
    query = "SELECT Temp_K, Cp_kJ_per_kg_K, Cv_kJ_per_kg_K, h_kJ_per_kg FROM IdealGasProperties_Air ORDER BY Temp_K"
    ...
    hs = np.array([row[3] for row in data])

    if Tactual in temps:
        # Exact match found
        index = np.where(temps == Tactual)[0][0]
        return {"Cp (kJ/kg*K)": float(cps[index]), "Cv (kJ/kg*K)": float(cvs[index]), "h (kJ/kg)":float(hs[index])}
        ...
        return {"Cp (kJ/kg*K)": float(Cp_interp), "Cv (kJ/kg*K)": float(Cv_interp), "h (kJ/kg)":float(h_interp)}
